...
void tsqr_func(cublasHandle_t cublas_handle, int share_memory_size, int m,
...
#ifdef TSDEBUG
        MatrixName = "before tsqr m < BLOCKSIZE A " + std::to_string(lda);
        writeMatrixToCsvV2(A, lda, m, n, fileName, MatrixName);
        MatrixName = "before tsqr m < BLOCKSIZE R " + std::to_string(ldr);
        writeMatrixToCsvV2(R, ldr, n, n, fileName, MatrixName);
#endif
        tsqr_kernel<<<1, blockDim, share_memory_size>>>(m, n, A, lda, R, ldr);
        cudaDeviceSynchronize();
#ifdef TSDEBUG
        MatrixName = "m < BLOCKSIZE A " + std::to_string(lda);
        writeMatrixToCsvV2(A, lda, m, n, fileName, MatrixName);
        MatrixName = "m < BLOCKSIZE R " + std::to_string(ldr);
        writeMatrixToCsvV2(R, ldr, n, n, fileName, MatrixName);
#endif
        return;
    }
    ...
    MatrixName = "before tsqr m > BLOCKSIZE A " + std::to_string(lda);
    writeMatrixToCsvV2(A, lda, m, n, fileName, MatrixName);
    MatrixName = "before tsqr m > BLOCKSIZE work " + std::to_string(ldwork);
    writeMatrixToCsvV2(work, ldwork, blockNum * n, n, fileName, MatrixName);
#endif
    ...
    MatrixName = "m > BLOCKSIZE A " + std::to_string(lda);
    writeMatrixToCsvV2(A, lda, m, n, fileName, MatrixName);
    MatrixName = "m > BLOCKSIZE work " + std::to_string(ldwork);
    writeMatrixToCsvV2(work, ldwork, blockNum * n, n, fileName, MatrixName);
#endif
...
#ifdef TSDEBUG
    cudaDeviceSynchronize();
    MatrixName = "Q in TSQR " + std::to_string(lda);
    writeMatrixToCsvV2(A, lda, m, n, fileName, MatrixName);
#endif
